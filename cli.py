...
QUICK_TEST_TEXT = "VoiceDNA quick natural check on Omarchy."
...
def test_natural_command(
    ...
    _run_speak(
        ...
        natural_voice=True,
        test_natural=True,
        show_backend=(True if quick_test else show_backend),
        low_vram=low_vram,
        save_wav=save_wav,
        play_audio=True,
    )
...
def doctor_natural_command(
    ...
    should_run = run_test or quick_test
    if not should_run:
    ...
    test_text = QUICK_TEST_TEXT if quick_test else ""
    _run_speak(
        text=test_text,
